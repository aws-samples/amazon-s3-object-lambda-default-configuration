...
Promise<PromiseResult<{}, AWSError> | null> {
  ...
  if (!objectResponse.ok) {
    ...
    return getResponseForS3Errors(s3Client, requestContext, objectResponse.status, originalObject)
  }
  ...
  if (transformedObject.hasError) {
    return getErrorResponse(s3Client, requestContext, ErrorCode.INVALID_REQUEST, String(transformedObject.errorMessage))
  } else if (transformedObject.object !== undefined) {
    return writeResponse(s3Client, requestContext, transformedObject.object)
  } else {
    return null
  }
}
 ...
 */
async function writeResponse (s3Client: S3, requestContext: GetObjectContext, transformedObject: Buffer): Promise<PromiseResult<{}, AWSError>> {
